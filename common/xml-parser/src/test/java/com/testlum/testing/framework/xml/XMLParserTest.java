...
import com.testlum.testing.framework.xml.model.ObjectFactory;
import com.testlum.testing.framework.xml.model.TestItem;
...
class XMLParserTest {
    ...
    @Test
    void processOutdatedNamespaceReportsExpectedNamespace() {
        File outdated = getResourceFile("test-schema/outdated-namespace-item.xml");
        XSDException exception = assertThrows(XSDException.class, () -> parser.process(outdated));
        assertTrue(exception.getMessage().contains("http://test.testlum.com/test-item-v1"));
        assertTrue(exception.getMessage().contains("http://test.testlum.com/test-item"));
        assertTrue(exception.getMessage().contains("update the xmlns"));
    }
