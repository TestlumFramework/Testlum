...
import com.knubisoft.testlum.testing.framework.report.testrails.model.ResultResponseDto;
import com.knubisoft.testlum.testing.framework.report.testrails.util.TestRailResponseJsonDeserializer;
...
public class TestRailAttachmentApiClient {
    ...
    private void sendAttachmentForResult(final Map<Integer, String> screenshotOfLastUnsuccessfulStep,
        ...
        try {
            ...
            ResponseEntity<String> getTestResponse = restTemplate
                    .exchange(getTestUrl, HttpMethod.GET, new HttpEntity<>(connectionService.buildHeaders()), String.class);
            log.info(TestRailConstants.LOG_FETCHING_TEST_SUCCESS_RESPONSE, resultDTO.getTestId(), getTestResponse.getBody());
            jsonDeserializer.extractIdFieldFromJson(getTestResponse.getBody(), TestRailConstants.CASE_ID)
