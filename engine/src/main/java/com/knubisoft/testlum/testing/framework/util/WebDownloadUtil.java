...
public final class WebDownloadUtil {
    ...
    public DownloadContext prepareDownload(final WebDriver driver, final File scenarioFile) {
    ...
    public void captureDownloadedFile(final DownloadContext ctx) {
    ...
    private List<Path> listNewFiles(final Path dir, final Set<String> beforeFileNames) {
    ...
    private Path newestReadyFile(final List<Path> candidates) {
    ...
    public void configureChromiumDownloadDir(final WebDriver driver, final Path dir) {
    ...
    public Set<String> snapshotFileNames(final Path dir) {
        try (Stream<Path> stream = Files.list(dir)) {
            return stream
                    .filter(Files::isRegularFile)
    ...
    public boolean isTempDownloadFile(final Path p) {
        String name = p.getFileName().toString().toLowerCase();
        return name.endsWith(".crdownload") || name.endsWith(".part") || name.endsWith(".tmp");
    }

    public long safeLastModified(final Path p) {
        try {
    ...
    public long safeSize(final Path p) {
        try {
            return Files.size(p);
        } catch (IOException e) {
            return 0L;
    ...
    public void registerDownloadedFile(final Path file) {
        if (file == null) {
            return;
        }
        try {
    ...
    public void cleanupDownloadedFiles(final boolean keepFiles) {
        try {
            if (!keepFiles) {
    ...
    private boolean tryExecuteCdp(final WebDriver driver,
